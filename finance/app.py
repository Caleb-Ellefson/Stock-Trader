...
def index():
    ...
    purchases_db = db.execute("SELECT symbol, SUM(SHARES) AS shares, price FROM purchases WHERE user_id = ? AND shares > 0 AND type = 'BUY' GROUP BY symbol", user_id)
...
def sell():
    ...
    if request.method == "GET":
        ...
        cash_db = db.execute("SELECT cash FROM users WHERE id = ?", user_id)
        cash = (cash_db[0]["cash"])
        symbols_user = db.execute("SELECT symbol FROM purchases WHERE user_id = ? GROUP BY symbol HAVING SUM(shares) > 0", user_id)

        return render_template("sell.html", database=purchases_db, cash=cash, symbols = [row["symbol"] for row in symbols_user])
        ...
        db.execute("INSERT INTO purchases (symbol, shares, price, user_id, type) VALUES (?, ?, ?, ?, 'SELL')", stock["symbol"], (-1 * quantity), stock["price"], user_id)
